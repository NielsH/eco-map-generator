...
- **Block & ore composition editor** — per biome, edit the ore **veins** (deposits) and **scatter** (strata) with sliders: block type, spawn chance, depth, vein size, noise frequency. Add/remove nodes per biome.
- **Live ore-distribution chart** — a violin/bar chart of where each material concentrates by biome and world height, updating as you edit — plus an **Open in ore visualizer** hand-off to the standalone [eco-biome-visualizer](https://github.com/NielsH/eco-biome-visualizer).
