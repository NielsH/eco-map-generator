...
- **Design a map (paint → exact world)** — draw the world you want and turn it into one that matches *exactly*. Paint **biomes, elevation, and rivers/water** on a toroidal canvas (brush, flood-fill, always-on edge-wrap with a live 2×2 seam preview, undo); elevation shows as a shaded relief map and painted areas get natural per-stroke roughness. **Import an image** to map its colours to biomes — with a colour-legend override, or a **Brightness** mode that maps luminance onto a terrain ramp for photos. **Preview it in 3D**, then **Export** a bundle (biome + height maps + config) for a companion C# tool (`EcoWorldGenCLI`, not included here) that drives Eco's real headless generator to write a stock-loadable `.eco` whose biomes/terrain match your drawing. Re-import a design `.zip` later to keep tuning. Requires a local Eco server build for the export step.
