...
(function () {
  ...
  function imageToMaps(res) {
    ...
    const OCEAN_DIST = 0.18 * res, EPOW = 2.0;                                    // reach full height ~18% inland; ^2 like Eco
    ...
    const ridged = (x, y) => { let s = 0, amp = 1, fr = 6, norm = 0; for (let o = 0; o < 4; o++) { const nz = vnR(x, y, fr, res); s += amp * (1 - Math.abs(2 * nz - 1)); norm += amp; amp *= 0.5; fr *= 2; } return s / norm; };   // ridged multifractal [0,1]
    const MAXH = 0.62;                                                           // interior peaks ~0.62 above sea
    for (let j = 0; j < n; j++) {
      ...
      const relief = (0.30 + 0.70 * ridged(x, y)) * MAXH;                         // ridged mountain ranges, biome-independent amplitude
      const nudge = (tgt[j] - 0.25) * 0.35;                                       // high-elevation biomes trend a little higher
      const above = Math.max(0.02, Math.min(maxE, relief + nudge + (vnR(x, y, 22, res) - 0.5) * 0.05));
      hf[j] = 0.5 + above * 0.5;                                                  // designer frac (0.5 = sea)
    }
    // per-cell water surface: nearest local land height (BFS carrying the min adjacent land height inward)
    const surf = new Float32Array(n).fill(-1); const lakeSet = new Set();
    ...
    // light smoothing only (soften pixel noise + the shoreline step) — keep the ridged relief intact
    boxBlurTor(hf, res, Math.max(2, Math.round(res / 80)), 1);
