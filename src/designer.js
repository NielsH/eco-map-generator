...
(function () {
  ...
  function imageToMaps(res) {
    ...
    // ---- real terrain relief: add multi-octave rolling hills on top of the per-biome base so the world
    //      isn't a flat plateau (broad P6 swells + medium + fine). Applied to land BEFORE lake analysis and
    //      the blur, so lake shores reflect the actual hilly height. Coastal lows clamp just above sea.
    const HILL_AMP = 0.26;
    for (let j = 0; j < n; j++) {
      if (land[j] !== 1) continue;
      const x = j % res, y = (j / res) | 0;
      const hill = 0.5 * vnR(x, y, 6, res) + 0.32 * vnR(x, y, 15, res) + 0.18 * vnR(x, y, 38, res);   // ~[0,1]
      hf[j] = Math.max(SEA + 0.006, hf[j] + (hill - 0.5) * HILL_AMP);
    }
    // ---- inland water -> lakes/rivers, sitting RELATIVE to their surroundings (a lake in the highlands
    //      sits high, not in a pit). Two kinds become fresh water: (a) any fresh-water-coloured region
    //      (even if it touches the sea, so coast rivers stay fresh), (b) salt water fully enclosed by land
    //      (not the main sea). Each body gets ONE flat surface just below its LOWEST surrounding biome's
    //      land height (so it never overflows and reads correctly across a biome border). This runs BEFORE
    //      the smoothing blur, and sets lake cells near their surface height, so shores are not pulled down.
    const water = new Uint8Array(n), bed = new Float32Array(n);
    ...
    for (const cells of groups) {
      let refMin = Infinity; const tally = {};
      for (const j of cells) for (const q of nbr4(j)) if (land[q] === 1) { const rh = hf[q]; if (rh < refMin) refMin = rh; tally[biome[q]] = (tally[biome[q]] || 0) + 1; }   // hf now = biome base + hills
      if (!isFinite(refMin)) refMin = 0.60;                                      // lake with no land shore -> mid elevation
      const surface = Math.max(SEA + 0.01, Math.min(0.985, refMin - 0.02));       // flat, below the lowest surrounding bank, above sea
      const bottom = Math.max(0.03, surface - 0.05);                             // bed below the surface
      let lb = SC.Grassland, bc = -1; for (const k in tally) if (tally[k] > bc) { bc = tally[k]; lb = +k; }   // majority shore biome
      const wval = Math.max(1, Math.min(255, Math.round((2 * surface - 1) * 255)));
      for (const j of cells) { biome[j] = lb; land[j] = 2; water[j] = wval; bed[j] = bottom; hf[j] = surface; }   // hf=surface so the blur keeps shores up
    }
    // smooth biome-edge steps into slopes (lakes now sit near their surface, so no deep pit pulls shores down)
    boxBlurTor(hf, res, Math.max(2, Math.round(res / 50)), 2);
    // finalize: lakes hold water above their (fixed) bed; other land keeps relief above sea; sea stays deep
    for (let j = 0; j < n; j++) {
      let h = land[j] === 2 ? bed[j] : (land[j] ? Math.max(0.505, hf[j]) : Math.min(hf[j], 0.44));   // hills already baked into hf
