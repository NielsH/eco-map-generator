...
(function () {
  ...
  let paintMode = 'biome';                     // 'biome' | 'elevation'
  ...
  const ECO_BIOME_ELEV = {
    Ocean: 0.30, Coast: 0.50, Grassland: 0.60, WarmForest: 0.62, ColdForest: 0.64, RainForest: 0.60,
    Desert: 0.56, Taiga: 0.70, Tundra: 0.74, Ice: 0.82, Wetland: 0.52,
  };
      ...
      '<div class="dsnCols">' +
        '<div class="dsnLeft">' +
          ...
          '<div class="dsnTools" style="margin-top:0">' +
            '<span class="dsnMini">Paint</span><span class="seg" id="dsnPaintMode"><button type="button" data-pm="biome" class="on">Biomes</button><button type="button" data-pm="elevation">Elevation</button></span>' +
          ...
          '</div>' +
          '<div class="dsnMini">Left-drag paints · right-drag erases (to ocean / to default height) · <b>flood fill</b> bucket-fills a biome region. In <b>Elevation</b> mode you sculpt height; unpainted areas use a biome-appropriate default.</div>' +
        '</div>' +
        '<div class="dsnRight">' +
  ...
  function pushUndo() { undoStack.push({ t: target.slice(), e: elev.slice(), p: elevPainted.slice() }); if (undoStack.length > 40) undoStack.shift(); }
  function undo() { if (!undoStack.length) return; const s = undoStack.pop(); target = s.t; elev = s.e; elevPainted = s.p; renderPaint(); }
  ...
  // biome-derived default elevation (0.5 = sea level) where the user hasn't painted, else the painted value
  function heightAt(x, y) {
    const i = y * G + x;
    if (elevPainted[i]) return elev[i];
    return ECO_BIOME_ELEV[CN[target[i]]] != null ? ECO_BIOME_ELEV[CN[target[i]]] : 0.5;
  }
  function drawHeight(canvas) {
    ...
    for (let y = 0; y < W; y++) { const gy = G - 1 - y; for (let x = 0; x < W; x++) {
      const b = Math.max(0, Math.min(255, Math.round(heightAt(x, gy) * 255))), o = (y * W + x) * 4;
      // tint water blue-ish, land by grayscale, so it reads as terrain
      const water = target[gy * G + x] === SC.Ocean;
      d[o] = water ? 40 : b; d[o + 1] = water ? 70 : b; d[o + 2] = water ? 120 : b; d[o + 3] = 255;
    } }
    ctx.putImageData(img, 0, 0);
  }
  ...
  async function buildBundleFiles() {
    ...
    const heightBin = new Uint8Array(G * G);    // per-cell height byte 0..255 (mod maps to [-1,1])
    for (let y = 0; y < G; y++) for (let x = 0; x < G; x++) {
      const i = y * G + x;
      biomeBin[i] = target[i];
      heightBin[i] = Math.max(0, Math.min(255, Math.round(heightAt(x, y) * 255)));
    }
    // PNGs kept for human inspection only (not read by the mod).
    const biomePng = await gridToPng((x, y) => ECO_BIOME_COLOR[CN[target[y * G + x]]] || ECO_BIOME_COLOR.Ocean);
    const heightPng = await gridToPng((x, y) => { const b = heightBin[y * G + x]; return [b, b, b]; });
    return {
      size: (cfg && findWorldWidth(cfg)) || 72,
      files: [
