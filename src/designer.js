...
(function () {
  ...
  function imageToMaps(res) {
    ...
    const bandRefH = cls => { const b = ECO_BIOME_ELEV[CN[cls]] || [0.52, 0.62]; return LAND_MEAN + ((b[0] + b[1]) / 2 - LAND_MEAN) * COMPRESS; };   // un-blurred land height for a biome
    const groups = [];
    {
    ...
    for (const cells of groups) {
      ...
      for (const j of cells) for (const q of nbr4(j)) if (land[q] === 1) { const rh = bandRefH(biome[q]); if (rh < refMin) refMin = rh; tally[biome[q]] = (tally[biome[q]] || 0) + 1; }
    ...
    for (let j = 0; j < n; j++) {
      ...
      let h = land[j] === 2 ? bed[j] : (land[j] ? Math.max(0.505, hf[j] + (fbmR(x, y, res) - 0.5) * 0.05) : Math.min(hf[j], 0.44));
