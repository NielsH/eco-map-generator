...
(function () {
  ...
  // over more ground: 2.68 world blocks per cell at 120 chunks, where every constant in imageToMaps was
  // tuned, and 4.46 at 200. That is what flattened a 4 km2 world - 62.5% of it came out as flat 5x5
  // building pads against stock's 36-39% - and what let the bed's ring steps grow big enough for the
  // mod's cliff pass to fill lakes with rock.
  //
  // Hold the PITCH instead of the grid. Only the coarsening is corrected: a world small enough to already
  // be finer than 2.68 keeps the 448 it has, so nothing at or below 120 chunks moves by a byte.
  const TUNED_WORLD = 1200;                 // the world imageToMaps' constants were tuned on: 120 chunks
  const IMPORT_BPC = TUNED_WORLD / 448;     // the pitch every constant in imageToMaps was tuned at
  const IMPORT_RES_MAX = 1024;              // measured: a 1.19 MB bundle on a 200-chunk map, cap 2 MB
  function importRes(worldBlocks) {
    if (!worldBlocks) return IMPORT_RES;
    return Math.max(IMPORT_RES, Math.min(IMPORT_RES_MAX, Math.round(worldBlocks / IMPORT_BPC)));
  }
  ...
  // The reaches are DISTANCES, in world blocks. As `res / 27` the valley reached 45 blocks on a 120-chunk
  // world and 74 on a 200-chunk one, and pulled 42% of that map's land toward one shoulder height.
  const VALLEY_RISE_MIN = 3, VALLEY_RISE_MAX = 6, VALLEY_BLOCKS = 45, SHORE_BLOCKS = 30, BLUFF_BLOCKS = 8.6;   // the rise in cells; the valley's reach, the shore cap's and the bluff's climb in world blocks
  ...
  function valleyBanks(res, n, land, wsurf, hf, prof, BPC) {
    const nbr4 = j => { const x = j % res, y = (j / res) | 0; return [((x + 1) % res) + y * res, ((x + res - 1) % res) + y * res, x + ((y + 1) % res) * res, x + ((y + res - 1) % res) * res]; };
    ...
    const VALLEY_REACH = Math.max(3, Math.round(VALLEY_BLOCKS / BPC));
    ...
    for (let p = 0; p < VALLEY_PASSES; p++) {
      for (let j = 0; j < n; j++) {
        ...
        // for the whole 16 m — the water was perched and the bank was what held it in.
          ...
          // Never leave it UNDER the water, the same bound the downward pull already carries. The pull
          // tapers with distance, so a cell far enough out only closes part of the gap - and ground that
          // stops below the surface beside it is not ground, it is the ledge this pass exists to remove.
          hf[j] = Math.max(Math.min(t, aim), near[j] + BLK);
        }
      }
    }
  }
  ...
  function capShoreBand(res, n, land, wsurf, hf, prof, aroundLand, SETTLE_MARGIN, BPC) {
    const nbr4 = j => { const x = j % res, y = (j / res) | 0; return [((x + 1) % res) + y * res, ((x + res - 1) % res) + y * res, x + ((y + 1) % res) * res, x + ((y + res - 1) % res) * res]; };
    const vshF = prof.vshF, vrsF = prof.vrsF, bluff = prof.bluff;
    const SHORE_REACH = Math.max(3, Math.round(SHORE_BLOCKS / BPC));
    ...
    const BLUFF_REACH = Math.max(2, BLUFF_BLOCKS / BPC), BLUFF_SETBACK = 0.9, SETBACK_VARY = 1.5;
  ...
  function imageToMaps(res, blocksPerCell) {
    ...
    // EPOW shapes the whole coastal climb. At 1.8 it is so convex that the first 40 blocks inland stay
    ...
    // RIDGE_FREQ is CYCLES ACROSS THE WORLD, so on a bigger map the same mountain range is drawn
    // proportionally bigger and the ground between crests proportionally flatter. Stock does the
    // opposite - VoronoiWorldGenerator scales its noise by worldWidth/72 - so a hill stays a hill's
    // size in blocks however large the world is.
    //
    // Measured on one drawing, each size at the pitch it is actually built at: mean rise per cell fell
    // from 0.33 at 120 chunks to 0.20 at 280, and flat share rose from 81% to 86%. A bigger world was
    // not getting more mountainous, it was getting emptier.
    //
    // Only the COARSENING is corrected. Below 120 chunks FREQ_SCALE clamps to 1, because scaling the
    // other way makes a small world's features bigger, and on a 72-chunk map - Eco's default - that
    // took the 90th percentile from 14 blocks to 20.
    //
    // Only THIS frequency. The fine noise and the low-ground swell scale with it to no measurable
    // effect, and scaling the warp and the peak field as well costs 2 blocks of median height at 200
    // chunks (10 -> 12) for 0.01 of rise - those two place features rather than texture them, and
    // making them denser just puts more peaks on the map.
    const FREQ_SCALE = Math.max(1, (res * BPC) / TUNED_WORLD);
    const RIDGE_FREQ = Math.max(1, Math.round(6 * FREQ_SCALE));   // finer than a terrace band just makes corduroy
    const RIDGE_OCTAVES = 5;
    const RIDGE_FALLOFF = 0.55;
    const FINE_AMP = 0.06;
    const LOW_FLOOR = 0.035;   // lowest land, in relief units: about two blocks over the sea
    const LOW_SWELL = 0.045;   // how far the floor swells above that, so low ground spreads over a few blocks
    const LOW_FREQ = 26;       // coarse: broad low plains, not a rash of pimples along the shore
    ...
    for (let j = 0; j < n; j++) {
      ...
      // The floor UNDULATES, and sits a couple of blocks higher than it did. A constant floor is where
      // the low ground piles up - a sixth of all land came out on it, every cell at the same single byte
      // over the sea - and that has two costs. The mod upscales the export by interpolating, so a cell
      // one byte over the sea drops under it wherever anything lower is next door: a third of the drawn
      // land never reached the world. And what survived was a plain at exactly one block, which the
      // first block of sea-rise takes. Stock's lowest band peaks a block above its minimum, because
      // nothing there is clamped to anything.
      const swell = vnR(x, y, LOW_FREQ, res) * 0.65 + vnR(x + 53.1, y + 17.7, LOW_FREQ * 2, res) * 0.35;
      const above = Math.max(LOW_FLOOR + LOW_SWELL * swell, (relief + nudge + (vnR(x, y, 24, res) - 0.5) * FINE_AMP) * maxE);
    ...
    // the river mouths are built on.
    {
      ...
      // BPC, not a hardcoded 1200: both of these windows are specified in WORLD BLOCKS, and dividing by
      // the wrong pitch makes them the wrong size everywhere except a 120-chunk world. On a 200-chunk
      // world this asked for 60 blocks and got 98.
      const cells = Math.max(4, Math.round(TREND_BLOCKS / BPC));
    ...
    let flatMask = null;   // how far each cell is levelled, read again by the cell relief below
    ...
    // and anything read straight off it gets printed into the terrain.
    {
      ...
      const cells = Math.max(3, Math.round(FLAT_BLOCKS / BPC));   // world blocks -> cells at THIS world's pitch
    ...
    // 5 import cells is ~13 world blocks. At 6.5 (~17) a 13x13-block pad fitted inside every cell, so a third
    // of the land sat on a level 35 m sheet whatever the cells' heights did; stock's polygons are 10-13.
    const CELL_SPACING = 5;         // cells across, in import cells
    let cellOff = null;             // each pixel's own cell offset, applied once the valley pass has run
    ...
    // corridor pass below runs it a second time with a narrower pair of those. `siteOffset(cx, cy, si)`,
    // if given, is a site's own departure from the field it read, handed back per pixel as `off`.
    const cellFlatten = (siteOk, cellOk, siteOffset) => {
      ...
      const siteOff = new Float32Array(cells * cells);
      for (let cy = 0; cy < cells; cy++) for (let cx = 0; cx < cells; cx++) {
        ...
        if (siteOk(si)) { siteV[c] = hf[si]; siteOff[c] = siteOffset ? siteOffset(cx, cy, si) : 0; siteUse[c] = 1; }
      }
      const flat = new Float32Array(hf), off = new Float32Array(n);
      for (let y = 0; y < res; y++) for (let x = 0; x < res; x++) {
        const j = y * res + x;
        ...
        if (best >= 0) { flat[j] = siteV[best]; off[j] = siteOff[best]; }
      }
      const soft = new Float32Array(flat);
      boxBlurTor(soft, res, 1, 1);
      return { flat, soft, off };
    };
    {
      const CELL_SHARP = 0.90;      // how much of the hard cell edge survives the smoothing
      // A cell is its OWN height, not a sample of the smooth field. Stock draws each polygon's elevation
      // with a random term of several blocks, so neighbours land on different terrace levels; sampled off
      // a smooth field, neighbours 17 blocks apart differed by a block and a half - half a level - and the
      // terrace merged them into sheets. Measured on the owner's 200-chunk map: 64% of the land inside a
      // level 13x13-block pad and one level running 233,000 columns, against stock's 25-28% and 34-41,000.
      // Stock has no less relief at any lag from 2.7 m to 43 m, only more of it (1.7x), as more risers of
      // the same size - so the offset is zero-mean noise, not a lift, and the median height does not move.
      //
      // Gated off over the first 45 blocks in from the sea, where stock's own noise is clamped under its
      // ocean-distance ceiling, and off the levelled biomes. A cell never goes DOWN past 5 blocks over the
      // sea: the +3 sea-rise loss stays at 16% (stock 26-33%) instead of climbing to 17-18%. Not 6 - that
      // takes the hollows beyond a lake's rim with it (verify-water's rim check: 4.0% against a 5% bound).
      const CELL_RELIEF = 3.0;         // blocks either way a cell departs from the smooth field
      const CELL_GATE_BLOCKS = 45;     // in from the sea, over which the offset fades in
      const CELL_FLOOR_BLOCKS = 5;     // over the sea; no cell is put down below it
      const isLand = j => land[j] === 1;
      const cellRelief = (cx, cy, si) => {
        const gate = Math.min(1, distO[si] * BPC / CELL_GATE_BLOCKS) * (1 - flatMask[si]);
        const off = (h2(cx + 5151, cy + 2323) - 0.5) * 2 * CELL_RELIEF * BLK * gate;
        return Math.max(Math.min(hf[si], 0.5 + CELL_FLOOR_BLOCKS * BLK), hf[si] + off) - hf[si];
      };
      const { flat, soft, off } = cellFlatten(isLand, isLand, cellRelief);
    ...
    const vshF = prof.vshF, vrsF = prof.vrsF, bluff = prof.bluff;
    valleyBanks(res, n, land, wsurf, hf, prof, BPC);
    // The offsets go on AFTER the valley pass, which pulls every cell within its reach toward one shoulder
    // height and would otherwise erase them - that pass is where the largest sheet came from (the 233,000
    // columns above were a river corridor). Faded in over the shoulder's own rise so the bank keeps the
    // profile the passes below were tuned to.
    {
      const CELL_TAPER = VALLEY_RISE_MAX;   // cells from the water over which a cell's own offset fades in
      const dw = new Int32Array(n).fill(-1), q = [];
      for (let j = 0; j < n; j++) if (land[j] === 2) { dw[j] = 0; q.push(j); }
      for (let k = 0; k < q.length; k++) { const c = q[k]; if (dw[c] >= CELL_TAPER + 1) continue;
        for (const nb of nbr4(c)) if (dw[nb] < 0 && land[nb] === 1) { dw[nb] = dw[c] + 1; q.push(nb); } }
      for (let j = 0; j < n; j++) {
        if (land[j] !== 1) continue;
        const t = dw[j] < 0 ? 1 : Math.max(0, Math.min(1, (dw[j] - 1) / CELL_TAPER));
        hf[j] += cellOff[j] * t;
      }
    }
    ...
    restoreShore();
    capShoreBand(res, n, land, wsurf, hf, prof, aroundLand, SETTLE_MARGIN, BPC);
  ...
  // 512 held the paint path to 3.91 blocks per cell on a 200-chunk world when it asks for 2. The maps are
  // a small part of the bundle and they compress well - three 1024-cell maps are 247 KB of a 1.19 MB
  // upload against the service's 2 MB limit - so the cap was costing detail it did not need to.
  const EXPORT_RES_MAX = 1024;
  ...
  function buildExportMaps() {
    ...
    valleyBanks(N, N * N, wf.land, wf.wsurf, fine, prof, worldBlocks / N);
    ...
    capShoreBand(N, N * N, wf.land, wf.wsurf, fine, prof, settled.aroundLand, settled.SETTLE_MARGIN, worldBlocks / N);
    restoreShoreLip(N, wf.land, wf.wsurf, fine);
    ...
    if (importedImg && !paintedSinceImport) {
      ...
      const iRes = importRes(wwCells);
      hi = imageToMaps(iRes, wwCells / iRes); biomeOut = hi.biome; heightOut = hi.height;
