...
(function () {
  ...
  function drawHeight(canvas) {
    ...
    for (let y = 0; y < W; y++) { const gy = G - 1 - y; for (let x = 0; x < W; x++) {
      const b = Math.max(0, Math.min(255, Math.round(f[gy * G + x] * 255))), o = (y * W + x) * 4;
      const water = target[gy * G + x] === SC.Ocean;   // tint water so it reads as terrain
  ...
  async function buildBundleFiles() {
    ...
    const field = computeHeightField();
    ...
    for (let i = 0; i < G * G; i++) {
      biomeBin[i] = target[i];
      heightBin[i] = Math.max(0, Math.min(255, Math.round(field[i] * 255)));
    }
