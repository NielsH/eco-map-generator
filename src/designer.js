...
(function () {
  ...
  function buildUI() {
      ...
      '<div class="dsnCols">' +
        '<div class="dsnLeft">' +
          ...
          '<div class="dsnTools">' +
            ...
            '<button id="dsnImport" title="Map an image\'s colors to the nearest biomes">📁 Import image</button>' +
    ...
    $('dsnClear').onclick = () => { pushUndo(); target.fill(SC.Ocean); elevPainted.fill(0); water.fill(0); renderPaint(); };
  ...
  // Load an image, stretch it to the world grid, and map each cell to the nearest biome color.
  function importImage(file) {
    ...
    img.onload = () => {
      ...
      const d = cx.getImageData(0, 0, G, G).data, count = {};
      for (let y = 0; y < G; y++) for (let x = 0; x < G; x++) {
        const o = (y * G + x) * 4;
        const cls = d[o + 3] < 128 ? SC.Ocean : nearestClass(d[o], d[o + 1], d[o + 2]);
      ...
      flashAnalysis('Imported image → nearest biomes (' + top + '). Edit or Analyze, then export.');
    };
