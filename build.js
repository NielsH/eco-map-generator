...
const OreVisual = (function () {
  ...
  function render() {
    ...
    const shAt = (i, d) => { const info = oi[i]; if (d < info.mn || d > info.mx || info.sh <= 0) return 0;
      ...
      // A vein was drawn at full strength whatever its parent, so one seeded in a layer that only exists in
      // 43% of columns looked as certain as one in a layer that always exists. Its SEED can only land where
      // its own stratum is the rock, so scale by the parent there too - it just does not carve to it,
      // since the blocks it then grows do overwrite whatever they reach.
      if (hasBase) { const pi = stratIdxOf.get(info.o.parent); if (pi != null) return info.sh * baseProb[pi][d]; }
      return info.sh; };
