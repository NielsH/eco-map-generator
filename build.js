...
const OreVisual = (function () {
  ...
  function veinNoteHtml(node) {
    ...
    // and it does not move with any setting - read first it has been taken for the thickness every time.
    if (relief > 0) h += '<span style="opacity:.7"> Depth varies: each deposit hangs under the surface of its own seed'
      + ' and the ground rolls over ' + relief + ' blocks, so the same layer turns up anywhere within about '
      + band + ' blocks of depth across the biome.</span>';
    return h; }
  ...
  function wireDetail(o) { const node = o.node;
    detailEl.querySelectorAll('input,select').forEach(inp => inp.addEventListener('input', () => {
      ...
      render(); renderList(); scheduleOreRender();
    }));
