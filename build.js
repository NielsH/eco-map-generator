...
const OreVisual = (function () {
  ...
  function render() {
    ...
    if (stripOn) {
      ...
      // The strip DOES show fills, so a grey footnote saying veins are missing reads as a technicality
      // rather than as "the biggest thing in this biome is not in this picture". It has been mistaken for
      // "there is no ore here" - say it at the top, in the ore colour, where the ore would have been.
      { const warn = '#c0705a';
        s += '<text x="' + (SX + SW / 2) + '" y="' + f1(TOPY + 14) + '" text-anchor="middle" font-size="10" font-weight="600" fill="' + warn + '" pointer-events="none" paint-order="stroke" stroke="' + cssv('--surf') + '" stroke-width="3">no veins in this view</text>';
        s += '<text x="' + (SX + SW / 2) + '" y="' + f1(TOPY + 26) + '" text-anchor="middle" font-size="9" fill="' + warn + '" pointer-events="none" paint-order="stroke" stroke="' + cssv('--surf') + '" stroke-width="3">fills only — veins are a world-wide pass</text>';
        s += '<text x="' + SX + '" y="' + f1(yAtDepth(rows) + 11) + '" font-size="9" fill="' + cM + '" pointer-events="none">select a vein to see one drawn to scale on the left</text>'; }
    }
    ...
    if (selBarInfo && selBarInfo.shape) {
      ...
      s += '<text x="' + f1(cx) + '" y="' + f1(capY) + '" text-anchor="middle" font-size="9.5" fill="' + cM + '" paint-order="stroke" stroke="' + cssv('--surf') + '" stroke-width="3">'
        + 'one deposit · ' + e.tall + ' tall × ' + e.wide + ' wide' + (e.shapes > 1 ? ' (1 of ' + e.shapes + ' shapes)' : '') + '</text>';
