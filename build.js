...
const OreVisual = (function () {
  ...
  function render() {
    ...
    // columns left to right: depth ruler | probability stack | one bar per vein/fill (its settings) | world-Y edge
    ...
    const floorD = surfMid;   // Y0 sits this deep under the mean surface; higher ground digs deeper
    const edgeBot = yAtDepth(Math.min(maxD, floorD) + 1);
    s += '<line x1="' + EX + '" y1="' + f1(colTop) + '" x2="' + EX + '" y2="' + f1(edgeBot) + '" stroke="' + cS + '"/>';
    for (let Y = 0; Y <= surfMid; Y += 10) { const d = surfMid - Y; if (d > maxD) continue; const yy = yAtDepth(d + 0.5), big = Y % 20 === 0;
      s += '<line x1="' + EX + '" y1="' + f1(yy) + '" x2="' + (EX + (big ? 6 : 3)) + '" y2="' + f1(yy) + '" stroke="' + cS + '"/>';
      if (big) s += '<text x="' + (EX + 9) + '" y="' + f1(yy + 3.5) + '" font-size="9.5" fill="' + cM + '">Y' + Y + '</text>'; }
    s += '<text x="' + (EX + 50) + '" y="' + f1(axMid) + '" fill="' + cS + '" font-size="10" text-anchor="middle" transform="rotate(-90 ' + (EX + 50) + ' ' + f1(axMid) + ')">world Y under the mean surface (Y' + surfMid + ')</text>';
    ...
    if (floorD <= maxD) { const fy = yAtDepth(floorD + 1);
      ...
      s += '<line x1="' + CX + '" y1="' + f1(fy) + '" x2="' + EX + '" y2="' + f1(fy) + '" stroke="' + cS + '" stroke-width="1"/><text x="' + (EX - 4) + '" y="' + f1(fy + 11) + '" text-anchor="end" font-size="9.5" fill="' + cM + '" pointer-events="none">Y0 under the mean surface · higher ground digs deeper</text>'; }
